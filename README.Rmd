...
The functions `init_exclude()`, `exclude()` and `get_exclude()` accept also parameter `e_name` that
can be used to have multiple independent exclude objects existing simultaneously.
The exclude objects are actually hidden global objects, and they can be referred to with the `e_name` parameter.
